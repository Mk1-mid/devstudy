  ...
  var module = {
    ...
    lessons: [
      ...
      },
      {
        ...
        theory: [
          {
            heading: "Operaciones CRUD con CSV/JSON",
            tag: "CRUD",
            content: "Aprenderás a crear, leer, actualizar y eliminar registros persistidos en CSV y JSON. Usa `csv.DictReader/DictWriter` para CSV y `json.load/dump` para JSON.",
            code: "import csv, json\n# ejemplo rápido: leer CSV y escribir JSON\nrows = []\nwith open('datos.csv','r',encoding='utf-8') as f:\n    reader = csv.DictReader(f)\n    for r in reader:\n        rows.append(r)\nwith open('datos.json','w',encoding='utf-8') as f:\n    json.dump(rows, f, ensure_ascii=False, indent=2)",
            callout: "💡 Mantén copias de seguridad antes de operaciones `w` para no perder datos."
          },
          {
            heading: "Buenas prácticas en CRUD",
            tag: "PRACTICAS",
            content: "Evita sobrescribir datos sin copia; valida entradas; maneja excepciones al parsear filas; procesa archivos grandes línea a línea para ahorrar memoria.",
            code: "# procesar archivo grande línea a línea\nwith open('grande.csv','r',encoding='utf-8') as f:\n    for line in f:\n        procesar(line)",
            callout: "⚠️ Antes de usar modo 'w' confirma que no perderás datos; usa 'a' o crea un archivo temporal y reemplázalo al final."
          },
          {
            heading: "Ejemplo: editar registro (actualizar)",
            tag: "UPDATE",
            content: "Estrategia común: leer todos los registros, modificar en memoria (o escribir a un archivo temporal) y luego reemplazar el original.",
            code: "import csv, tempfile, os\nwith open('datos.csv','r',encoding='utf-8') as rf, tempfile.NamedTemporaryFile('w', delete=False, encoding='utf-8') as wf:\n    reader = csv.DictReader(rf)\n    writer = csv.DictWriter(wf, fieldnames=reader.fieldnames)\n    writer.writeheader()\n    for r in reader:\n        if r['id'] == '3':\n            r['name'] = 'Nuevo'\n        writer.writerow(r)\nos.replace(wf.name, 'datos.csv')",
            callout: null
          }
        ],
        exercises: [
